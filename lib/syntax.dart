...
class TypeDefinition {
  ...
  String jsonParseExpression(String key, bool privateField) {
    ...
    if (isPrimitive) {
      ...
      //  $fieldKey = List<dynamic>.from(json["$key"] != null && (json["$key"] is bool) == false ? json["$key"] : []);
      // }""";
      //   }
      if (name == "List") {
        return """$fieldKey = json['$key']  is bool  ? [] : json["$key"];""";
      }
  ...
  String toJsonExpression(String key, bool privateField) {
    ...
    if (isPrimitive) {
      return "if ($thisKey != null) data['$key'] = $thisKey;";
    } else if (name == 'List') {
      // class list
...
class ClassDefinition {
  ...
  void _addTypeDef(TypeDefinition typeDef, StringBuffer sb) {
    ...
    if (typeDef.subtype != null) {
      ...
      //Luanvm-subtype change to dynamic
      sb.write('<dynamic>');
    }
  }
